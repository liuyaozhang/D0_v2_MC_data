...
int main(int argc, char** argv)
{
   ...
   for(int ipt=0; ipt<ana::nPt; ipt++){
      ...
      h1DMass[ipt] = new TH1D(Form("h1DMass_pt%d", ipt), "", 60, 1.7, 2.0);
   ...
   for(long int ientry=0; ientry<nentries; ientry++){
      ...
      if((ientry +1)%1000000==0) cout << "Working on event#" << ientry << endl; 
      auto bytes = evt->GetEntry(ientry);//get ith event. 
      if(bytes == 0 || bytes == -1) {
   ...
   for(int imass=0; imass<ana::nMass; imass++){
      for(int ipt=0; ipt<ana::nPt; ipt++){
         ...
         //check the number of content in signal and background; 
         //=======================
	 //for(int i=1;i<hSignal_D0[imass][ipt].at("smallDCA")->GetNbinsX();i++){
	 //        for(int j=1;j<hSignal_D0[imass][ipt].at("smallDCA")->GetNbinsY();j++){
	 //       	 sumS_flow[imass][ipt] = hSignal_D0[imass][ipt].at("smallDCA")-> GetBinContent(i,j); 
	 //       	 sumB_flow[imass][ipt] = hBackground_D0[imass][ipt].at("smallDCA")-> GetBinContent(i,j); 
	 //       	 cout << "sumS_flow[imass][ipt]" << " = " << sumS_flow[imass][ipt] << endl; 
	 //       	 cout << "sumB_flow[imass][ipt]" << " = " << sumB_flow[imass][ipt] << endl; 
	 //        }
	 //}
