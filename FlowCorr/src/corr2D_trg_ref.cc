...
int main(int argc, char** argv)
{
   ...
   list<vector<TVector3>> pVectList_ref;
   vector<float> effVect_ref;
   list<vector<float>> effVectList_ref;
   ...
   //
   // temporary vectors
   TVector3 p_dau1(0, 0, 0), p_dau2(0, 0, 0), p_d0(0, 0, 0), p_ref(0, 0, 0);
   ...
   for(long int ientry=0; ientry<nentries; ientry++){
      ...
      for(unsigned int itrack=0; itrack<evt->CandSizeTrk(); itrack++){
         ...
         for(unsigned int id0=0; id0<pVect_d0.size(); id0++){
            if(ana::rejectDaughter_){
               if(fabs(evt->EtaTrk(itrack) - pVect_dau1_d0.at(id0).Eta())<0.03
                     && fabs(evt->PhiTrk(itrack) - pVect_dau1_d0.at(id0).Phi())<0.03)
                  pass_rejecDaughter = false;
               if(fabs(evt->EtaTrk(itrack) - pVect_dau2_d0.at(id0).Eta())<0.03
                     && fabs(evt->PhiTrk(itrack) - pVect_dau2_d0.at(id0).Phi())<0.03)
                  pass_rejecDaughter = false;
            }
         }
      ...
      if(pVectList_ref.size() == ana::nMixedEvts){
         for(unsigned int itrg=0; itrg<nMult_ref; itrg++){
            // simultaneously read momentum and efficiency
            auto ievt_eff_ass = effVectList_ref.begin();
            for(auto ievt_p_ass= pVectList_ref.begin(); ievt_p_ass != pVectList_ref.end(); ievt_p_ass++){
      ...
      if(pVectList_ref.size() == ana::nMixedEvts){
         ...
         pVectList_ref.erase(pVectList_ref.begin());
         pVectList_ref.push_back(pVect_ref);
         effVectList_ref.erase(effVectList_ref.begin());
         effVectList_ref.push_back(effVect_ref);
      } else{
